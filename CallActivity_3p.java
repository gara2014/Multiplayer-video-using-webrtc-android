...
import cn.xiaoyang123.videochat.R;
...
public class CallActivity extends Activity implements PeerConnectionClient.PeerConnectionEvents,
  ...
  private SurfaceViewRenderer remote1RenderScreen;
  private SurfaceViewRenderer remote2RenderScreen;
  private SurfaceViewRenderer remote3RenderScreen;
  ...
  private PercentFrameLayout remote3RenderLayout;
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    remote1RenderScreen = (SurfaceViewRenderer) findViewById(R.id.remote1_video_view);
    remote2RenderScreen = (SurfaceViewRenderer) findViewById(R.id.remote2_video_view);
    remote3RenderScreen = (SurfaceViewRenderer) findViewById(R.id.remote3_video_view);
    localRenderLayout = (PercentFrameLayout) findViewById(R.id.local_video_layout);
    remote1RenderLayout = (PercentFrameLayout) findViewById(R.id.remote1_video_layout);
    remote2RenderLayout = (PercentFrameLayout) findViewById(R.id.remote2_video_layout);
    remote3RenderLayout = (PercentFrameLayout) findViewById(R.id.remote3_video_layout);
    ...
    remote1RenderScreen.setOnClickListener(listener);
    remote2RenderScreen.setOnClickListener(listener);
    remote3RenderScreen.setOnClickListener(listener);
    remoteRenderers.add(remote1RenderScreen);
    ...
    remote1RenderScreen.init(rootEglBase.getEglBaseContext(), null);
    remote2RenderScreen.init(rootEglBase.getEglBaseContext(), null);
    remote3RenderScreen.init(rootEglBase.getEglBaseContext(), null);

    localRender.setZOrderMediaOverlay(true);
    localRender.setEnableHardwareScaler(true /* enabled */);
    remote1RenderScreen.setEnableHardwareScaler(true /* enabled */);
    remote2RenderScreen.setEnableHardwareScaler(true /* enabled */);
    remote3RenderScreen.setEnableHardwareScaler(true /* enabled */);
    updateVideoView();
    ...
    peerConnectionParameters =
        new PeerConnectionParameters(intent.getBooleanExtra(EXTRA_VIDEO_CALL, true), loopback,
            ...
            intent.getIntExtra(EXTRA_VIDEO_BITRATE, 160), intent.getStringExtra(EXTRA_VIDEOCODEC),
...
//    }
  ...
  private void updateVideoView() {
    remote1RenderLayout.setPosition(0, 0, 48, 48);
    remote1RenderScreen.setScalingType(scalingType);
    remote1RenderScreen.setMirror(false);

    remote2RenderLayout.setPosition(52, 0, 48, 48);
    remote2RenderScreen.setScalingType(scalingType);
    remote2RenderScreen.setMirror(false);

    remote3RenderLayout.setPosition(0, 52, 48, 48);
    remote3RenderScreen.setScalingType(scalingType);
    remote3RenderScreen.setMirror(false);
...
//    }
    ...
    remote1RenderScreen.requestLayout();
    remote2RenderScreen.requestLayout();
    remote3RenderScreen.requestLayout();
  }
  ...
  private void startCall() {
    ...
    roomSignalClient.joinRoom(roomConnectionParameters.roomId);
  ...
  private void callConnected(PeerConnectionClient.PeerConnectionNode peerConnectionNode) {
    ...
    //peerConnectionClient.enableStatsEvents(true, STAT_CALLBACK_PERIOD, peerConnectionNode);
  }
...
//    }
    if (roomSignalClient != null) {
      ...
      roomSignalClient.leaveRoom(roomConnectionParameters.roomId);
    ...
    if (remote1RenderScreen != null) {
      remote1RenderScreen.release();
      remote1RenderScreen = null;
    }
    if (remote2RenderScreen != null) {
      remote2RenderScreen.release();
      remote2RenderScreen = null;
    }
    if (remote3RenderScreen != null) {
      remote3RenderScreen.release();
      remote3RenderScreen = null;
    }
...
//    });
  ...
  private int getIdleRemoteRenderIndex() {
    for (int i = 0; i < Max_Remote_Count; i++) {
      boolean find = false;
      for (int j = 0; j < peerConnectionClient.peerConnectionNodes.size(); j++) {
        if (peerConnectionClient.peerConnectionNodes.get(j).remoteRenderIndex == i) {
          find = true;
          break;
        }
      }
  ...
  private SurfaceViewRenderer getIdleRemoteRender(int idx) {
    if (idx == 0) return remote1RenderScreen;
    else if(idx == 1) return remote2RenderScreen;
    else if(idx == 2) return remote3RenderScreen;
    else return null;
  }
        ...
        if (error == 0) {
          ...
          if (other != null) {
            for (int i = 0; i < other.size(); i++) {
              ...
              int idleRemoteRenderIndex = getIdleRemoteRenderIndex();
              SurfaceViewRenderer remoteRender = getIdleRemoteRender(idleRemoteRenderIndex);
              if (remoteRender != null) {
                peerConnectionClient.createPeerConnection(remoteRender, true, ru.userId, idleRemoteRenderIndex);
    ...
    runOnUiThread(new Runnable() {
      ...
      public void run() {
        ...
        if (peerConnectionNode == null) {
          ...
          SurfaceViewRenderer remoteRender = getIdleRemoteRender(idleRemoteRenderIndex);
          if (remoteRender != null) {
            peerConnectionClient.createPeerConnection(remoteRender, false, srcUserId, idleRemoteRenderIndex);
          }
          isInitiator = false;
        }
